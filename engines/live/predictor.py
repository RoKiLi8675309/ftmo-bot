...
class Signal:
    ...
    V20.6 FIX: Added signal_id for precise execution tracking to prevent
    consecutive_loss memory leaks when trades are blocked downstream.
    """
...
class ProbabilityCalibrator:
    ...
    def calibrate(self, raw_prob: float) -> float:
        # V13.1 FIX: If we return < min_conf (e.g. 0.50) here, the bot NEVER takes a trade,
        # meaning it never learns, meaning it stays immature forever.
        # We MUST return 1.0 to force exploration until it has 50 valid outcomes.
...
class AdaptiveTripleBarrier:
    """
    V20.6 SPREAD TRAP CURE & CHOP LABELING.
    Enforces wide barriers to escape broker slippage and tracks execution state
    to prevent memory leaks on rejected signals.
    """
    ...
    def add_trade_opportunity(self, features: Dict[str, float], entry_price: float, current_atr: float, 
        ...
        self.buffer.append({
            'signal_id': signal_id,
            'is_executed': False,  # V20.6: Only flips to True if Engine confirms dispatch
        ...
        while self.buffer:
            ...
            if buy_status != 0 or sell_status != 0 or is_expired:
                ...
                resolved.append((
                    ...
                    pred_proba,
                    proposed_ret,
                    is_executed  # V20.6 FIX: Explicit Execution Feedback Tuple
                ))
            else:
                active.append(trade)
...
class MultiAssetPredictor:
    ...
    def __init__(self, symbols: List[str], threshold_map: Optional[Dict[str, float]] = None):
        ...
        risk_mult_conf = float(risk_conf.get('stop_loss_atr_mult', 1.5)) # V20.6 tighter baseline 
        ...
        self.bb_window = 20
        self.bb_std = 1.5 # V20.6 Wider bands to prevent chop entries
        ...
        self.hurst_breakout = float(phx_conf.get('hurst_breakout_threshold', 0.52)) # V20.6 Stricter Trends
        ...
        for s in symbols:
            ...
            s_reward = tbm_conf.get('barrier_width', 3.0) # V20.6 forces 1:3 baseline to beat spread
    ...
    def confirm_execution(self, symbol: str, signal_id: str):
        """
        V20.6 IPC FIX: Called by the LiveTradingEngine (via internal Queue) when a signal 
        successfully passes all downstream Risk/Exposure Gates and is dispatched to MT5.
        Prevents consecutive_loss memory leaks.
        """
    ...
    def _train_on_bar(self, symbol: str, bar: VolumeBar):
        ...
        # ============================================================
        # V13.2 SMART SIGNAL MATRIX (WARMUP - HIGH VOL VETO)
        # ============================================================
        ...
        # Generate fake UUID during warmup to satisfy the interface
    ...
    def process_bar(self, symbol: str, bar: VolumeBar, context_data: Dict[str, Any] = None) -> Optional[Signal]:
        ...
        try:
            ...
            if resolved_labels:
                for (stored_feats, optimal_label, optimal_ret, past_action, past_prob, past_ret, is_executed) in resolved_labels:
                    ...
                    # ONLY adjust performance state on trades that genuinely touched MT5 metal
                    if is_executed:
        ...
        # ============================================================
        # D. ADD TRADE OPPORTUNITY (WITH BROKER REALITY & UUID)
        # ============================================================
        self.labelers[symbol].add_trade_opportunity(
        ...
        # ============================================================
        # F. ML CONFIRMATION & SIGNAL DISPATCH (V13.1 EXPECTANCY CURE)
        # ============================================================
        try:
