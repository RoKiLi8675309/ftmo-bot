...
class Signal:
    ...
    V20.8 FIX: Added signal_id for precise execution tracking to prevent
...
class ProbabilityCalibrator:
    ...
    def calibrate(self, raw_prob: float) -> float:
        # V20.8 FIX: ML Confidence Pegging Bug.
        # Return the RAW PROBABILITY instead of a flat fallback so the base model can filter early!
...
class AdaptiveTripleBarrier:
    """
    V20.8 SPREAD TRAP CURE & CHOP LABELING.
    ...
    def add_trade_opportunity(self, features: Dict[str, float], entry_price: float, current_atr: float, 
        ...
        self.buffer.append({
            'signal_id': signal_id,
            'is_executed': False,  # V20.8 IPC Fix: Only flips to True if Engine confirms dispatch
    ...
    def resolve_labels(self, current_high: float, current_low: float, current_close: float = None, 
        ...
        while self.buffer:
            ...
            if self.horizon_type == 'TIME':
                # V20.8 Fix: True wall-clock duration checking
            ...
            if buy_status != 0 or sell_status != 0 or is_expired:
                ...
                resolved.append((
                    ...
                    pred_proba,
                    proposed_ret,
                    is_executed  # V20.8 Fix: Execution feedback tuple inclusion
                ))
            else:
                active.append(trade)
...
class MultiAssetPredictor:
    ...
    def confirm_execution(self, symbol: str, signal_id: str):
        """
        V20.8 IPC FIX: Called by the LiveTradingEngine (via internal Queue) when a signal 
    ...
    def process_bar(self, symbol: str, bar: VolumeBar, context_data: Dict[str, Any] = None) -> Optional[Signal]:
        ...
        try:
            ...
            if resolved_labels:
                # V20.8 IPC Fix: Unpack 7th element (is_executed)
                for (stored_feats, optimal_label, optimal_ret, past_action, past_prob, past_ret, is_executed) in resolved_labels:
    ...
    def save_state(self):
        """
        V20.8 FIX: Wrap each save operation in a try/except block. 
        This ensures that if one component fails to pickle (e.g., a locked ThreadPool), 
        the core ML models and state buffers still save successfully.
        """
        for sym in self.symbols:
