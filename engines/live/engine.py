        ...
        while self.running:
            try:
                ...
                if bar:
                    ...
                    if signal:
                        # V20.18 FIX: Prevent deadlocks by utilizing non-blocking queue put
                        try:
                            ...
                            worker_log.warning(f"⚠️ Signal Queue Full! Dropping signal for {symbol} to prevent Logic Worker deadlock.")
                    ...
                    for stream, messages in response:
                        for msg_id, data in messages:
                            ...
                            if symbol and ticket:
                                with self.stats_lock:
                                    # V20.18 FIX: Strict Deduplication Guard to prevent SQN and Loss counter inflation
                                    if ticket not in self.daily_execution_stats[symbol]['tickets']:
                                        # 1. Safely Append to Performance Stats (SQN)
                                        self.performance_stats[symbol].append({'pnl': net_pnl, 'action': action})
                                        
                                        # 2. Update Limits and Sets
                                        self.daily_execution_stats[symbol]['pnl'] += net_pnl
                                        self.daily_execution_stats[symbol]['tickets'].add(ticket)
                                        ...
                                        # 3. Handle Penalties (Revenge Guard) only once per actual MT5 closure
